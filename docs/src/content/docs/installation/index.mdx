...
<Steps>
 ...
 1. Start by adding the GPG key for the repository:
    ...
    sudo curl -fsSL https://apt-swarm.ktrain5369.dev/apt-swarm-public-key.asc \
    ...
    <Tabs>
        <TabItem label="Debian">
          ...
          echo "deb [signed-by=/etc/apt/keyrings/apt-swarm.asc] \ 
            https://apt-swarm.ktrain5369.dev bookworm main" \
        ...
        <TabItem label="Ubuntu">
          ```bash
          echo "deb [signed-by=/etc/apt/keyrings/apt-swarm.asc] \
            https://apt-swarm.ktrain5369.dev/ noble main" \
            | sudo tee /etc/apt/sources.list.d/apt-swarm.list
          ```
        </TabItem>
    </Tabs>
